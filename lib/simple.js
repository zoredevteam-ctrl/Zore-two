...
// Función helper para asegurar que el directorio tmp existe
async function ensureTmpDir() {
  const tmpDir = path.join(__dirname, '../tmp')
  try {
    await fs.promises.access(tmpDir)
    console.log('Directorio tmp existe:', tmpDir)
    return true
  } catch {
    console.log('Directorio tmp no existe, creando:', tmpDir)
    try {
      // Intentar con promises.mkdir primero
      await fs.promises.mkdir(tmpDir, { recursive: true, mode: 0o755 })
      console.log('Directorio tmp creado exitosamente con promises.mkdir')
      return true
    } catch (err) {
      console.error('Error con promises.mkdir, intentando con mkdirSync:', err.message)
      try {
        // Fallback a mkdirSync
        fs.mkdirSync(tmpDir, { recursive: true, mode: 0o755 })
        console.log('Directorio tmp creado exitosamente con mkdirSync')
        return true
      } catch (syncErr) {
        console.error('Error creando directorio tmp con mkdirSync:', syncErr.message)
        console.error('Ruta completa:', tmpDir)

        // Último intento: verificar si el directorio padre existe
        const parentDir = path.dirname(tmpDir)
        try {
          const parentExists = fs.existsSync(parentDir)
          console.log('Directorio padre existe:', parentExists, parentDir)
          if (parentExists) {
            console.log('Permisos del directorio padre:', fs.statSync(parentDir).mode.toString(8))
          }
        } catch (parentErr) {
          console.error('Error verificando directorio padre:', parentErr.message)
        }

        throw new Error('No se pudo crear el directorio temporal después de múltiples intentos: ' + syncErr.message)
      }
    }
  }
}
