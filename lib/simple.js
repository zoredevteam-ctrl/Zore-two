    ...
    if (m.key) {
        ...
        if (m.isGroup && m.sender?.endsWith('@lid')) {
            try {
                ...
                const found = groupMeta.participants.find(p => p.id === m.sender || p.lid === m.sender)
    ...
    if (m.message) {
        ...
        if (quoted?.quotedMessage) {
            ...
            if (m.quoted.sender?.endsWith('@lid')) {
                try {
                    const groupMeta = await conn.groupMetadata(m.chat)
                    const found = groupMeta.participants.find(p => p.id === m.quoted.sender || p.lid === m.quoted.sender)
                    if (found?.jid) m.quoted.sender = found.jid
                } catch {}
            }
