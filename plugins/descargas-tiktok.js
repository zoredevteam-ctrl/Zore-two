...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) throw m.reply(`${emoji} uwu favor, ingresa un enlace de *TikTok*.`);
  conn.sendMessage(m.chat, { react: { text: "🕒", key: m.key } });
  let d2 = await fetch(`https:                                                             
  let dp = await d2.json()
  const doc = {
    audio: { url: dp.results.audio },
    mimetype: 'audio/mp4',
    fileName: `//eliasar-yt-api.vercel.app/api/search/tiktok?query=${text}`)
  let dp = await d2.json()
  const doc = {
    audio: { url: dp.results.audio },
    mimetype: 'audio/mp4',
