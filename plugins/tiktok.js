...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    try {
      console.log(chalk.yellow(`[TT] Buscando en: ${endpoint}`)); // Log pa debug
...
✦ *Fecha:* ${created_at || 'Del pasado, darling'}`.trim();
        ...
        if (type === 'image') {
          ...
          if (audioUrl) {
            console.log(chalk.green(`[TT] Mandando audio: ${audioUrl}`)); // Log pa debug
          ...
          console.log(chalk.green(`[TT] Mandando video: ${videoUrl}`)); // Log pa debug
...
✦ *Audio:* \( {v.music?.title || `[ \){v.author?.nickname || 'No disponible'}] original sound - ${v.author?.unique_id || 'unknown'}`}`.trim();
        ...
        console.log(chalk.green(`[TT] Mandando álbum con ${medias.length} videos`)); // Log pa debug
        await client.sendAlbumMessage(m.chat, medias, { quoted: m });
      }
    } catch (e) {
      console.error(chalk.red(`[TT ERROR] Falló el fetch o send: ${e.message}`)); // Log pa debug error
